...
void oh_upsample_filter_block_luma_h_x2_sse(int16_t *dst, ptrdiff_t dststride, uint8_t *_src, ptrdiff_t srcstride,
...
void oh_upsample_filter_block_luma_v_x2_sse(uint8_t *_dst, ptrdiff_t _dststride, int16_t *_src, ptrdiff_t srcstride,
...
void oh_upsample_filter_block_cr_v_x2_sse(uint8_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
        int y_BL, int x_EL, int y_EL, int width, int height, int widthEL, int heightEL,
        const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ret;
...
void oh_upsample_filter_block_luma_h_x1_5_sse(int16_t *dst, ptrdiff_t dststride, uint8_t *_src, ptrdiff_t srcstride,
            int x_EL, int x_BL, int width, int height, int widthEL,
            const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info) {
    int x, y, ref, ret;
...
void oh_upsample_filter_block_luma_v_x1_5_sse(uint8_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
            int y_BL, int x_EL, int y_EL, int width, int height, int widthEL, int heightEL,
            const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ret, ret0;
...
void oh_upsample_filter_block_cr_h_x1_5_sse(int16_t *dst, ptrdiff_t dststride, uint8_t *_src, ptrdiff_t srcstride,
            int x_EL, int x_BL, int width, int height, int widthEL,
            const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ref, ret;
...
void oh_upsample_filter_block_cr_v_x1_5_sse(uint8_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
        int y_BL, int x_EL, int y_EL, int width, int height, int widthEL, int heightEL,
        const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
...
void oh_upsample_filter_block_luma_v_x2_sse_16(uint16_t *_dst, ptrdiff_t _dststride, int16_t *_src, ptrdiff_t srcstride,
            ...
            const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ret;
    __m128i x1, x2, x3, x4, x5, x6, x7, x8, x0, c[5], add, ceil;
    int16_t  *src;
    _dststride /= sizeof(uint16_t);
    uint16_t  *dst    = (uint16_t *)_dst + y_EL * _dststride + x_EL;
    int  shift = 10;
    add  = _mm_set1_epi32((1 << (shift - 1)));
...
void oh_upsample_filter_block_cr_v_x2_sse_16(uint16_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
        ...
        const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ret;
    __m128i x1, x2, x3, x4, x0, c[4], add, ceil;
    int16_t  *src;
...
DECLARE_ALIGNED(16, static const int16_t, up_sample_filter_luma_x1_5_h_sse_16[12][16] )= /* 0, 11, 5 */
{
    {   0,   0,  -1,   3,  -1,   4,   0,   0,  -1,   3,  -1,   4,   0,   0,  -1,   3},
    {   0,  64,  -8,  26, -11,  52,   0,  64,  -8,  26, -11,  52,   0,  64,  -8,  26},
    {   0,   0,  52, -11,  26,  -8,   0,   0,  52, -11,  26,  -8,   0,   0,  52, -11},
...
DECLARE_ALIGNED(16, static const int16_t, up_sample_filter_chroma_x1_5_h_sse_16[6][16] )= /* 0, 11, 5 */
{
    {  0, 64, -2, 20, -6, 52,  0, 64, -2, 20, -6, 52,  0, 64, -2, 20},
    {  0,  0, 52, -6, 20, -2,  0,  0, 52, -6, 20, -2,  0,  0, 52, -6},
    { -2, 20, -6, 52,  0, 64, -2, 20, -6, 52,  0, 64, -2, 20, -6, 52},
...
DECLARE_ALIGNED(16, static const uint8_t, masks_16[3][16] )= {
    {0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03, 0x02, 0x03, 0x04, 0x05, 0x04, 0x05, 0x06, 0x07},
    {0x00, 0x01, 0x02, 0x03, 0x02, 0x03, 0x04, 0x05, 0x04, 0x05, 0x06, 0x07, 0x04, 0x05, 0x06, 0x07},
    {0x00, 0x01, 0x02, 0x03, 0x02, 0x03, 0x04, 0x05, 0x02, 0x03, 0x04, 0x05, 0x04, 0x05, 0x06, 0x07}
};

DECLARE_ALIGNED(16, static const uint8_t, masks2_16[3][16] )= {
    {0x04, 0x05, 0x06, 0x07, 0x06, 0x07, 0x08, 0x09, 0x08, 0x09, 0x0a, 0x0b, 0x08, 0x09, 0x0a, 0x0b},
    {0x06, 0x07, 0x08, 0x09, 0x08, 0x09, 0x0a, 0x0b, 0x08, 0x09, 0x0a, 0x0b, 0x0a, 0x0b, 0x0c, 0x0d},
    {0x06, 0x07, 0x08, 0x09, 0x06, 0x07, 0x08, 0x09, 0x08, 0x09, 0x0a, 0x0b, 0x0a, 0x0b, 0x0c, 0x0d}
};
...
void oh_upsample_filter_block_luma_h_x1_5_sse_16(int16_t *dst, ptrdiff_t dststride, uint16_t *_src, ptrdiff_t srcstride,
    ...
    c[1]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[1]);
    c[2]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[2]);
    c[3]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[3]);

    c[4]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[4]);
    c[5]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[5]);
    c[6]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[6]);
    c[7]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[7]);

    c[8]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[8] );
    c[9]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[9] );
    c[10] = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[10]);
    c[11] = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[11]);

    c2[0]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[0]+1);
    c2[1]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[1]+1);
    c2[2]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[2]+1);
    c2[3]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[3]+1);

    c2[4]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[4]+1);
    c2[5]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[5]+1);
    c2[6]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[6]+1);
    c2[7]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[7]+1);

    c2[8]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[8]+1 );
    c2[9]  = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[9]+1 );
    c2[10] = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[10]+1);
    c2[11] = _mm_load_si128((__m128i *) up_sample_filter_luma_x1_5_h_sse_16[11]+1);
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x += 8 ) {
            ...
            mask = _mm_load_si128((__m128i *) masks2_16[ret]);

            x12 = _mm_shuffle_epi8(x1, mask);
            x22 = _mm_shuffle_epi8(x2, mask);
            x32 = _mm_shuffle_epi8(x3, mask);
            x42 = _mm_shuffle_epi8(x4, mask);

            mask = _mm_load_si128((__m128i *) masks_16[ret]);
            ...
            ret *= 4;
            x1 = _mm_madd_epi16(x1,c[ret]  );
            x2 = _mm_madd_epi16(x2,c[ret+1]);
            x3 = _mm_madd_epi16(x3,c[ret+2]);
            x4 = _mm_madd_epi16(x4,c[ret+3]);

            x12 = _mm_madd_epi16(x12,c2[ret]  );
            x22 = _mm_madd_epi16(x22,c2[ret+1]);
            x32 = _mm_madd_epi16(x32,c2[ret+2]);
            x42 = _mm_madd_epi16(x42,c2[ret+3]);
...
void oh_upsample_filter_block_cr_h_x1_5_sse_16(int16_t *dst, ptrdiff_t dststride, uint8_t *_src, ptrdiff_t srcstride,
    ...
    c[1]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[1]);
    c[2]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[2]);
    c[3]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[3]);
    c[4]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[4]);
    c[5]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[5]);

    c2[0]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[0]+1);
    c2[1]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[1]+1);
    c2[2]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[2]+1);
    c2[3]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[3]+1);
    c2[4]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[4]+1);
    c2[5]  = _mm_load_si128((__m128i *) up_sample_filter_chroma_x1_5_h_sse_16[5]+1);
...
void oh_upsample_filter_block_luma_v_x1_5_sse_16(uint16_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
            int y_BL, int x_EL, int y_EL, int width, int height, int widthEL, int heightEL,
            const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
    int x, y, ret, ret0;
    __m128i x1, x2, x3, x4, x5, x6, x7, x8, x0, c[9], add, ceil;
    int16_t  *src;
    dststride /= sizeof(uint16_t);
    uint16_t  *dst    = (uint16_t *)_dst + y_EL * dststride + x_EL;
    uint8_t  shift = 10;
...
void oh_upsample_filter_block_cr_v_x1_5_sse_16(uint16_t *_dst, ptrdiff_t dststride, int16_t *_src, ptrdiff_t srcstride,
        int y_BL, int x_EL, int y_EL, int width, int height, int widthEL, int heightEL,
        const struct HEVCWindow *Enhscal, struct UpsamplInf *up_info){
