...
static int decode_pic_timing(HEVCContext *s)
{
    ...
    if (sps->vui.frame_field_info_present_flag) {
        ...
        switch(s->picture_struct) {
            ...
            case  4 : s->interlaced = 1; av_log(s->avctx, AV_LOG_DEBUG, "bottom field, top field, in that order\n"); break;
        ...
        if (s->picture_struct == 1 ||
            s->picture_struct == 3 || s->picture_struct == 11)
            s->field_order = AV_FIELD_TT;
        else if (s->picture_struct == 2 ||
                 s->picture_struct == 4 || s->picture_struct == 12)
